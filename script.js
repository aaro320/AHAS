...
document.getElementById("headAvatarInput").addEventListener("change", function (e) {
  const file = e.target.files[0];
  if (file) {
    const reader = new FileReader();
    reader.onload = function (event) {
      const img = new Image();
      img.onload = () => { headAvatarImage = img; };
...
function update() {
  ...
  if (hitSelf) {
    if (hasShield) {
      hasShield = false;
      document.getElementById("powerupInfo").textContent = "🛡️ Shield Broke!";
      return; // Cancel movement tick to absorb hit
    } else {
      gameOver();
      return;
    }
...
function gameLoop(now) {
  ...
  if (powerupEnd && now >= powerupEnd) {
    ...
    // Fix Bug 1: Ensure food exists after magnet ends
    if (foods.length === 0) {
      spawnFood();
    }
  }
...
function drawHeadAvatar(head) {
  const size = 0.88;
  const x = head.x + (1 - size) / 2;
  const y = head.y + (1 - size) / 2;

  ctx.save();
  ctx.beginPath();
  ctx.arc(head.x + 0.5, head.y + 0.5, size / 2, 0, Math.PI * 2);
  ctx.clip();

  ctx.drawImage(headAvatarImage, x, y, size, size);
  ctx.restore();
}
